...
const Dashboard: React.FC = () => {
  ...
  const [healthScore, setHealthScore] = useState<string>(t('dashboard.loading') || 'Loading...');
  const [lastCheckup, setLastCheckup] = useState<string>(t('dashboard.loading') || 'Loading...');
      ...
      try {
        ...
        if (prescriptionsSnap.exists()) {
          ...
          if (active.length > 0) {
            ...
            setLastCheckup(t('dashboard.noActivePrescriptions') || 'No active prescriptions');
          }
        ...
        if (profileSnap.exists()) {
          ...
          setHealthScore(profile.healthScore || t('dashboard.notAvailable') || 'N/A');
        } else {
          setHealthScore(t('dashboard.notAvailable') || 'N/A');
        }
      } catch (err) {
  ...
  const stats = [
    {
      name: t('dashboard.stats.nextAppointment'),
      value: nextAppointment
        ? `${nextAppointment.preferredDate}, ${nextAppointment.preferredTime}`
        : t('dashboard.stats.noUpcomingAppointment'),
      icon: Calendar,
      color: 'bg-blue-500'
    },
    {
      name: t('dashboard.stats.activePrescriptions'),
      value: `${activePrescriptions.length}`,
      icon: Pill,
      color: 'bg-green-500'
    },
    {
      name: t('dashboard.stats.healthScore'),
      value: healthScore,
      icon: Heart,
      color: 'bg-red-500'
    },
    {
      name: t('dashboard.stats.lastCheckup'),
      value: lastCheckup,
      icon: Activity,
      color: 'bg-purple-500'
    }
  ...
  const quickActions = [
    { 
      name: t('dashboard.quickActions.bookAppointment'), 
      icon: Calendar, 
      href: '/book-appointment', 
      color: 'bg-blue-600' 
    },
    { 
      name: t('dashboard.quickActions.emergencyServices'), 
      icon: AlertTriangle, 
      href: '/emergency', 
      color: 'bg-red-600' 
    },
    { 
      name: t('dashboard.quickActions.viewPrescriptions'), 
      icon: FileText, 
      href: '/prescriptions', 
      color: 'bg-green-600' 
    },
    { 
      name: t('dashboard.quickActions.updateProfile'), 
      icon: Users, 
      href: '/profile', 
      color: 'bg-purple-600' 
    }
    ...
    <Layout>
      <div className="py-6">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
          ...
          {/* Quick Actions */}
          <div className="mt-8">
            <h2 className="text-lg font-medium text-gray-900 mb-4">{t('dashboard.quickActionsTitle')}</h2>
                  ...
                  {appointments.map((appointment, index) => (
                    <div key={index} className="border-l-4 border-blue-400 pl-4">
                      <p className="text-sm font-medium text-gray-900">
                        {appointment.symptoms || t('dashboard.generalConsultation')}
                      </p>
